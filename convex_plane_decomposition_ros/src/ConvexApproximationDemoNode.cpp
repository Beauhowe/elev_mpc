...
void callback(const convex_plane_decomposition_msgs::msg::PlanarTerrain::SharedPtr msg) {
...
int main(int argc, char** argv) {
  ...
  auto node = std::make_shared<rclcpp::Node>("convex_approximation_demo_node");

  auto positionPublisher = node->create_publisher<geometry_msgs::msg::PointStamped>("queryPosition", 1);
  auto projectionPublisher = node->create_publisher<geometry_msgs::msg::PointStamped>("projectedQueryPosition", 1);
  auto convexTerrainPublisher = node->create_publisher<geometry_msgs::msg::PolygonStamped>("convex_terrain", 1);
  auto terrainSubscriber = node->create_subscription<convex_plane_decomposition_msgs::msg::PlanarTerrain>(
      "/convex_plane_decomposition_ros/planar_terrain", 1, &callback);
  ...
  while (rclcpp::ok()) {
    {
      ...
      if (planarTerrainPtr) {
        ...
        header.stamp = rclcpp::Time(static_cast<int64_t>(planarTerrainPtr->gridMap.getTimestamp()));
